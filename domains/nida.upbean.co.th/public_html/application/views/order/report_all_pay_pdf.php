...
if (!empty($datas)){
    foreach ($datas as $data){
        if($first == 0 || (($y_point + 42) > 297)) {
            ...
            $pdf->SetFont('b', '', 17 );
            $pdf->SetXY( 0, $y_point );
            $pdf->MultiCell(210, 10, U2T("รายงานสรุปการชำระเงิน"), 0, "C");
            ...
            $pdf->MultiCell($col_w_2, 10, U2T("วันที่"), 0, "C");

            $pdf->setXY($col_x_3,$y_point);
            $pdf->MultiCell($col_w_3, 10, U2T("จำนวนเงิน"), 0, "R");
        ...
        $pdf->setXY($col_x_2,$y_point);
        $pdf->MultiCell($col_w_2, 10, U2T($this->center_function->ConvertToThaiDate($data['created_at'],1,0)), 0, "C");

        $pdf->setXY($col_x_3,$y_point);
        $pdf->MultiCell($col_w_3, 10, U2T(number_format($data['amount'],2)), 0, "R");
        $total += $data['amount'];
    }
} else {
    $y_point = 10;
...
$pdf->SetFont('b', '', 15 );
$pdf->setXY($col_x_3,$y_point);
$pdf->MultiCell($col_w_3, 10, U2T("รวม   ".number_format($total,2)), 0, "R");
